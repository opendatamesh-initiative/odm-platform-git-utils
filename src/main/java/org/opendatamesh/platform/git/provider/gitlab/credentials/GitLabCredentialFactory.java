package org.opendatamesh.platform.git.provider.gitlab.credentials;
...
public abstract class GitLabCredentialFactory {
    private static final String CREDENTIAL_TYPE = "x-odm-gpauth-type";

    private GitLabCredentialFactory() {
        //DO NOTHING
    }
    ...
    public static GitProviderCredential createCredentials(HttpHeaders headers) {
        ...
        throw new GitProviderConfigurationException("Unsupported Authentication Type for GitLab: " + type);
    }

    private static GitLabPatCredential buildPat(HttpHeaders headers) {
        ...
        return new GitLabPatCredential(token);
    }
}
