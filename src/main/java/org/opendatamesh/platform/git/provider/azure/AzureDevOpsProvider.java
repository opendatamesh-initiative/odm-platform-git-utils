...
import org.opendatamesh.platform.git.provider.azure.resources.getcurrentuser.AzureGetCurrentUserMapper;
import org.opendatamesh.platform.git.provider.azure.resources.getcurrentuser.AzureGetCurrentUserUserResponseRes;
import org.opendatamesh.platform.git.provider.azure.resources.getrepository.AzureGetRepositoryMapper;
...
import org.opendatamesh.platform.git.provider.azure.resources.listbranches.AzureListBranchesMapper;
import org.opendatamesh.platform.git.provider.azure.resources.listcommits.AzureListCommitsCommitListRes;
import org.opendatamesh.platform.git.provider.azure.resources.listcommits.AzureListCommitsCommitRes;
import org.opendatamesh.platform.git.provider.azure.resources.listcommits.AzureListCommitsMapper;
import org.opendatamesh.platform.git.provider.azure.resources.listrepositories.AzureListRepositoriesMapper;
...
import org.opendatamesh.platform.git.provider.azure.resources.listtags.AzureListTagsMapper;
...
public class AzureDevOpsProvider implements GitProvider {
    ...
    public AzureDevOpsProvider(String baseUrl, RestTemplate restTemplate, GitProviderCredential credential) throws GitProviderConfigurationException {
    ...
    public Page<Commit> listCommits(Repository repository, CommitListFilter filter, Pageable page) {
        try {
            ...
            if (filter instanceof CommitListRangeFilter r) {
                String fromType = r.from().getRefType().name().toLowerCase();
                String toType = r.to().getRefType().name().toLowerCase();
                uriData = buildUriTemplateAndVariablesListCommitsWithFromOrToFilters(repository, new FromOrToCommitFilters(r.from().getRefValue(), r.to().getRefValue(), fromType, toType), page);
