package org.opendatamesh.platform.git.provider.bitbucket.resources.getrepository;
...
public abstract class BitbucketGetRepositoryMapper {

    /**
     * Maps BitbucketGetRepositoryRepositoryRes to internal Repository model
     */
    public static Repository toInternalModel(BitbucketGetRepositoryRepositoryRes repoRes,
        ...
        if (repoRes.getLinks() != null && repoRes.getLinks().getClone() != null) {
            cloneUrlHttp = repoRes.getLinks().getClone().stream()
                    ...
                    .findFirst()
                    .map(BitbucketGetRepositoryLinkRes::getHref)
                    .orElse(null);
    ...
    /**
     * Creates BitbucketGetRepositoryRepositoryRes from internal Repository model
     */
    public static BitbucketGetRepositoryRepositoryRes fromInternalModel(Repository repository) {
        if (repository == null) {
            return null;
        }
        
        BitbucketGetRepositoryRepositoryRes repoRes = new BitbucketGetRepositoryRepositoryRes();
            ...
            for (ProviderCustomResourceProperty prop : repository.getProviderCustomResourceProperties()) {
                if (BitbucketRepositoryExtension.PROJECT.equals(prop.getName()) && prop.getValue() != null) {
                    try {
                        ...
                        repoRes.setProject(mapper.treeToValue(prop.getValue(), BitbucketGetRepositoryRepositoryProjectRes.class));
                    } catch (Exception e) {
        ...
        if (repository.getDefaultBranch() != null) {
            repoRes.setMainbranch(new BitbucketGetRepositoryMainBranchRes(repository.getDefaultBranch()));
        }
        
        // Set owner (minimal - only UUID)
        if (repository.getOwnerId() != null) {
            BitbucketGetRepositoryUserRes owner = new BitbucketGetRepositoryUserRes();
            owner.setUuid(repository.getOwnerId());
            repoRes.setOwner(owner);
        }
        
        // Build links from clone URLs
        List<BitbucketGetRepositoryLinkRes> cloneLinks = new ArrayList<>();
        if (repository.getCloneUrlHttp() != null) {
            cloneLinks.add(new BitbucketGetRepositoryLinkRes("https", repository.getCloneUrlHttp()));
        }
        if (repository.getCloneUrlSsh() != null) {
            cloneLinks.add(new BitbucketGetRepositoryLinkRes("ssh", repository.getCloneUrlSsh()));
        }
        if (!cloneLinks.isEmpty()) {
            repoRes.setLinks(new BitbucketGetRepositoryLinksRes(null, null, cloneLinks));
        }
     ...
     */
    private static List<ProviderCustomResourceProperty> buildAdditionalProperties(BitbucketGetRepositoryRepositoryRes repoRes) {
